...
const texts = {
  de: {
    ...
    adminBundleOptions: 'Optionen / Varianten (z. B. für 2 Wochen, S, M, L)',
  ...
  fr: {
    ...
    adminBundleOptions: 'Options / variantes (p. ex. pour 2 semaines, S, M, L)',
...
function parseBundleMeta(row){
  ...
  if(raw.startsWith(META_PREFIX)){
    try{
      ...
      const bundleOptions = directBundleOptions.length ? directBundleOptions : (Array.isArray(meta?.bundle_options) && meta.bundle_options.length ? meta.bundle_options : options);
      const slotType = row?.slot_type === 'bundle' || meta?.slot_type === 'bundle' ? 'bundle' : 'normal';
      return {
        slot_type: slotType,
        show_info: typeof meta?.show_info === 'boolean' ? meta.show_info : (rawShowInfo === undefined || rawShowInfo === null ? slotType === 'bundle' : Boolean(rawShowInfo)),
...
function formatProductInfoLines(product){
  ...
  return String(raw || '')
    ...
    .map(line => line.trim())
    .filter(Boolean)
    .map(line => line.replace(/(\d+)\s*\*\s*(\d+)/g, '$1×$2').replace(/\s{2,}/g, ' '));
}
function renderProductInfoContent(product){
  const lines = formatProductInfoLines(product);
  if(lines.length){
...
function bundleOptionList(product){
  const source = Array.isArray(product?.bundle_options) && product.bundle_options.length
    ...
    : (Array.isArray(product?.quantity_options) && product.quantity_options.length ? product.quantity_options : [2,3,4]);
  const list = source.map((item, index)=>normalizeBundleOption(item, index)).filter(Boolean);
  return list.length ? list : bundleOptionsFromInput('2x,3x,4x');
}
function bundleOptionLabel(option){
  if(!option) return '';
  return String((state.lang === 'fr' ? option.label_fr : option.label_de) || option.label_de || option.label_fr || `${option.factor}x`);
}
function ensureBundleSelection(product){
  if(product?.slot_type !== 'bundle') return null;
  const options = bundleOptionList(product);
  let index = Number(state.bundleSelections?.[product.id]);
  if(!Number.isInteger(index) || index < 0 || index >= options.length){
    const legacyMultiplier = Number(state.bundleSelections?.[product.id]);
    index = options.findIndex((opt)=>Number(opt.factor) === legacyMultiplier);
    if(index < 0) index = 0;
  }
  state.bundleSelections[product.id] = index;
  return options[index];
}
function selectedBundleMultiplier(product){
  if(product?.slot_type !== 'bundle') return 1;
  return Number(ensureBundleSelection(product)?.factor || 1);
}
function selectedBundleLabel(product){
  if(product?.slot_type !== 'bundle') return '';
  return bundleOptionLabel(ensureBundleSelection(product));
}
...
function cartEntryKey(entry){
  if(entry && typeof entry === 'object') return `${entry.productId || entry.id}::${entry.kind || 'normal'}::${entry.multiplier || 1}::${entry.optionLabel || ''}`;
  return String(entry);
}
...
function cartEntryOptionLabel(entry){
  if(entry && typeof entry === 'object') return String(entry.optionLabel || '');
  return '';
}
...
function addBundleProduct(productId){
  ...
  const selected = ensureBundleSelection(product);
  const multiplier = Number(selected?.factor || selectedBundleMultiplier(product));
  const optionLabel = bundleOptionLabel(selected);
  state.cart.push({ productId: String(product.id), kind: 'bundle', multiplier, optionLabel });
  save(); render();
}
function setBundleOption(productId, value){
  ...
  if(!Number.isInteger(index) || index < 0 || index >= options.length) index = 0;
  state.bundleSelections[product.id] = index;
  save(); render();
}
...
function addAdminSlot(){
  ...
  products.push({
    ...
    option_label: { de: '', fr: '' },
    quantity_options: [{label_de:'2x',label_fr:'2x',factor:2},{label_de:'3x',label_fr:'3x',factor:3},{label_de:'4x',label_fr:'4x',factor:4}],
    bundle_options: [{label_de:'2x',label_fr:'2x',factor:2},{label_de:'3x',label_fr:'3x',factor:3},{label_de:'4x',label_fr:'4x',factor:4}]
  });
    ...
    if(response.ok && data.success !== false && Array.isArray(data.products) && data.products.length){
      ...
      state.catalog.products.forEach((product)=>{
        if(product?.slot_type === 'bundle'){
          state.bundleSelections[product.id] = 0;
        }
      });
    }
  }catch(error){
...
function readAdminProductRowsDirectFromDom(){
  ...
  return products.map((product, index) => {
    ...
    return {
      ...
      bundle_options: qtyOptions
    };
  });
}

async function saveAdminProducts(forcedRows = null){
  const rows = Array.isArray(forcedRows) ? forcedRows : readAdminProductRowsDirectFromDom();
...
function cartItemsDetailed(){
  ...
  return state.cart.map((entry)=>{
    ...
    return {
      ...
      qtyLabel: kind === 'bundle' ? (cartEntryOptionLabel(entry) || selectedBundleLabel(product) || `${multiplier}x`) : ''
    };
  }).filter(Boolean);
}
function cartGrouped(){
  ...
  for (const p of cartItemsDetailed()){
    const key = `${p.id}::${p.kind || 'normal'}::${p.multiplier || 1}::${p.qtyLabel || ''}`;
      ...
      <div class="machine"><div class="machine-red"><div class="machine-inner">
        ...
        <div><div class="grid">
          ${currentProducts().map(p=>{
            ...
            const currentOption = isBundle ? ensureBundleSelection(p) : null;
          ...
          <div class="slot ${isBundle ? 'slot-bundle' : ''} ${isSoldOut(p) ? 'is-sold-out' : ''} ${isSelected ? 'selected' : ''}" data-id="${p.id}" role="button" tabindex="0" aria-label="${escapeAttr(displayName)}">
            ...
            ${productShowsInfo(p) ? `<button class="slot-info-strip" type="button" data-slot-info="${p.id}">${escapeHtml(localizedBundleLabel(p))}</button>` : ''}
            <div class="select-light ${isBundle ? 'select-light-bundle' : ''}">
              ${isBundle ? `<select class="slot-bundle-select" data-slot-option-select="${p.id}" aria-label="${t('slotChooseOption')}">
                ${bundleOptions.map((opt, optIndex)=>`<option value="${optIndex}" ${optIndex===currentOptionIndex?'selected':''}>${escapeHtml(bundleOptionLabel(opt))}</option>`).join('')}
              </select>` : '<span></span>'}
            </div>
          </div>`;
          }).join('')}
        ...
        <aside class="side">
          ...
          <div class="cartbox cartbox-compact">
            ...
            <div class="cart-list">
              ...
              <div class="cart-item">
                <div><strong>${item.name[state.lang]}</strong>${item.kind==='bundle' && item.qtyLabel ? `<div class="cart-subnote">${escapeHtml(item.qtyLabel)}</div>` : ''}<div>x${item.qty}</div></div>
...
function buildOrderPayload(){
  ...
  return {
    ...
    items: grouped.map(item => ({
      product_id: item.id,
      slot_code: item.slot,
      product_name: item.kind === 'bundle' && item.qtyLabel ? `${item.name[state.lang]} (${item.qtyLabel})` : item.name[state.lang],
      quantity: item.qty,
...
async function uploadCmsImageFile(file, slotLabel){
  if(!file) return '';
  const allowed = ['image/jpeg','image/png','image/webp','image/gif'];
  ...
  const form = new FormData();
  form.append('file', file);
  form.append('slot', slotLabel || 'cms');
  const response = await fetch('/.netlify/functions/upload-product-image', { method:'POST', body: form, credentials:'same-origin' });
  const data = await response.json().catch(()=>({}));
  if(!response.ok || data.success === false || !data.publicUrl) throw new Error(data.error || 'Upload fehlgeschlagen');
  return data.publicUrl;
}
async function uploadCmsDesignImage(field, file){
  if(!file || !field) return;
  state.admin.productsMessage = 'CMS Hintergrundbild wird hochgeladen …'; state.admin.loginError=''; save(); render();
  try{
    const publicUrl = await uploadCmsImageFile(file, `cms-design-${field}`);
    state.admin.design = state.admin.design || {};
    state.admin.design[field] = publicUrl;
    state.admin.productsMessage = 'CMS Hintergrundbild hochgeladen. Bitte Design speichern.';
    save(); render();
  }catch(error){ state.admin.loginError = error.message || 'Upload fehlgeschlagen'; save(); render(); }
}
async function uploadCmsBlockBackgroundImage(pageIndex, blockIndex, file){
  if(!file) return;
  const page = state.admin.pages?.[pageIndex];
  const block = page ? ensurePageMeta(page).blocks?.[blockIndex] : null;
  if(!block) return;
  state.admin.productsMessage = 'Block Hintergrundbild wird hochgeladen …'; state.admin.loginError=''; save(); render();
  try{
    const publicUrl = await uploadCmsImageFile(file, `cms-block-bg-${page.slug || pageIndex}`);
    const freshPage = state.admin.pages?.[pageIndex];
    const freshBlock = freshPage ? ensurePageMeta(freshPage).blocks?.[blockIndex] : null;
    if(freshBlock) freshBlock.bgImage = publicUrl;
    state.admin.productsMessage = 'Block Hintergrundbild hochgeladen. Bitte Design speichern.';
    save(); render();
  }catch(error){ state.admin.loginError = error.message || 'Upload fehlgeschlagen'; save(); render(); }
}
...
function blockOuterStyle(block, align='left'){
  ...
  const fontSize = cssLen(block?.fontSize, ''); if(fontSize) styles.push(`font-size:${escapeAttr(fontSize)}`);
  if(['400','500','600','700','800','900','normal','bold'].includes(String(block?.fontWeight || ''))) styles.push(`font-weight:${escapeAttr(block.fontWeight)}`);
  const borderWidth = cssLen(block?.borderWidth, '');
  if(borderWidth){ styles.push(`border:${escapeAttr(borderWidth)} solid ${escapeAttr(block?.borderColor || 'rgba(255,255,255,.22)')}`); }
  if(block?.shadow === 'soft') styles.push('box-shadow:0 16px 34px rgba(0,0,0,.22)');
  if(block?.shadow === 'strong') styles.push('box-shadow:0 24px 60px rgba(0,0,0,.38)');
  const minHeight = cssLen(block?.minHeight, ''); if(minHeight) styles.push(`min-height:${escapeAttr(minHeight)}`);
...
function newPageBlock(type='text'){
  const base = { id: `blk_${Date.now()}_${Math.random().toString(16).slice(2)}`, type, active:true, collapsed:false, align:'left', bgColor:'', textColor:'', padding:'', radius:'', maxWidth:'', marginTop:'', marginBottom:'', bgImage:'', bgSize:'cover', bgPosition:'center center', fontSize:'', fontWeight:'', borderWidth:'', borderColor:'', shadow:'none', minHeight:'' };
